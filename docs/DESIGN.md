...
- **콘솔은 문장 종류를 가르지 않고 `run_script` 하나로 보낸다.** 예전에는 첫 키워드로 조회(`run_query`)와 실행(`run_execute`)을 갈랐는데, 그러면 여러 문장을 넣었을 때 **첫 결과셋만 오고 나머지가 사라진다** — 서버는 다 실행했는데 화면에 안 나와 "일부가 실행되지 않았다"고 오해하게 된다. `run_query`/`run_execute` 는 드라이버 내부 조회·실행용으로만 남는다.
- 드라이버는 결과셋을 **경계째로** 모은다. sqlx 는 `fetch_many` 가 주는 `Either::Left`(=`QueryResult` 완료 신호)를 결과셋 구분점으로 쓰고, tiberius 는 `into_first_result()` 대신 `into_results()` 를 쓴다 — 전자가 바로 이 버그의 원인이었다.
- **행 수 제한은 결과셋마다 따로 건다.** 전체 합으로 자르면 앞 결과셋이 한도를 다 먹어 뒤 결과셋이 통째로 비어 버린다.
- 결과셋이 **둘 이상이면 결과 영역에 탭**을 띄운다(하나뿐이고 쓰기도 없으면 탭을 그리지 않는다 — 없어도 되는 UI 를 늘리지 않는다). 결과셋이 하나도 없으면(쓰기만 한 스크립트) 영향 행 수 합계를 보여 준다.
- **조회와 쓰기가 섞이면 둘 다 알린다.** 결과 표가 떴다고 영향 행 수를 숨기면 무엇이 바뀌었는지 알 길이 없다. 탭 줄 오른쪽에 "N행 영향"을 함께 띄우고, 상태바·토스트 요약에도 둘 다 넣는다.
- 쓰기 결과는 상태바뿐 아니라 **토스트로도** 알린다. 상태바는 화면 맨 아래라 놓치기 쉬운데, 몇 행이 바뀌었는지는 실행 직후 반드시 확인해야 하는 정보다.

### 변경 행 보기 (쓰기가 무엇을 바꿨는지)

"3행 영향"만으로는 **어느 행이 어떤 값으로** 바뀌었는지 알 수 없다. 콘솔 툴바의 "변경 행 보기"를 켜면 쓰기 문장에 변경 행 반환 절을 끼워 넣어 실제 행을 결과 탭으로 받는다(`db/script.rs::with_change_output`).

- **기본은 꺼 둔다.** 사용자 SQL 을 고쳐 보내는 일이라 매번 말없이 하지 않는다. 켠 상태로 실행이 실패하면 오류 메시지에 "절이 추가된 상태"임을 덧붙인다 — 이걸 모르면 멀쩡한 문장이 왜 깨졌는지 알 수 없다(트리거가 걸린 테이블은 SQL Server 가 `OUTPUT` 을 거부한다).
- **실제로 나간 SQL 을 `ScriptResult::sql` 로 돌려준다.** 우리가 고쳐 보냈다면 무엇이 실행됐는지 확인할 수 있어야 한다. 로그 패널은 원문 대신 이 SQL 을 남긴다.
- **배치 전체가 쓰기일 때만 손댄다.** SELECT 가 섞이면 돌아온 결과셋 중 어느 것이 변경 내역인지 가릴 수 없어, 컬럼 이름을 붙이거나 영향 행 수를 세는 근거가 사라진다. 못 고치는 배치는 그냥 두면 원래대로 실행되므로 **애매하면 포기한다**.
- 자리를 잘못 잡아도 데이터는 어긋나지 않는다 — 구문 오류로 문장이 실패할 뿐 아무것도 반영되지 않는다. `OUTPUT` 은 무엇이 바뀌는지를 바꾸지 않는다.
- 방언별로 얻는 것이 다르다.

| DB | 절 | UPDATE 에서 얻는 것 |
|----|----|--------------------|
| SQL Server | `OUTPUT deleted.*, inserted.*` (문장 **중간**, FROM/WHERE 앞) | **변경 전·후 모두** |
| PostgreSQL · SQLite | `RETURNING *` (문장 끝) | 변경 **후만** (표준에 전을 주는 방법이 없다) |
| MySQL | 없음 | 지원하지 않음 — 토글을 비활성화한다 |

- `deleted.*, inserted.*` 는 두 쪽 컬럼 이름이 그대로 겹친다(`id, name, id, name`). 어느 쪽이 전인지 모르면 값을 봐도 소용없으므로 앞 절반에 `이전.`, 뒤 절반에 `이후.` 를 붙인다(`label_before_after`). 절반끼리 이름이 맞지 않으면 우리가 만든 결과셋이 아니므로 손대지 않는다.
- 사용자가 직접 적은 `OUTPUT`/`RETURNING` 도 결과셋을 돌려준다(`may_return_rows` 가 문장 시작 키워드뿐 아니라 이 절도 본다). 이미 절이 있으면 우리가 덧붙이지 않는다 — 원하는 컬럼을 고른 것이다.

### SQL Server 의 배치(`GO`)와 tiberius 의 제약

SQL Server 만 `db/script.rs` 의 스크립트 스캐너를 거친다. 두 가지 때문이다.

- **`GO` 는 T-SQL 이 아니라 클라이언트 지시어다.** SSMS·DataGrip 에서 복사해 온 스크립트에 그대로 들어 있는데, 서버로 보내면 "Could not find stored procedure 'GO'" 로 실패한다. 보내기 전에 배치로 잘라야 한다. 문자열·주석 안의 `GO` 와 `GOTO` 는 구분자가 아니므로 스캐너가 문맥을 추적한다. 반복 횟수(`GO 5`)는 **무시하고 한 번만** 실행한다 — 조용히 여러 번 실행하는 쪽이 훨씬 위험하다.
- **tiberius 는 결과셋과 영향 행 수를 함께 주는 API 가 없다.** `simple_query` 는 결과셋만, `execute` 는 행 수만 준다(DONE 토큰을 다루는 `TokenStream` 이 `pub(crate)` 라 직접 합칠 수도 없다). 그래서 배치마다 `script::may_return_rows` 로 하나를 고른다. 판단은 **결과셋 쪽으로 치우쳐 둔다** — 잘못 골라도 영향 행 수를 못 보는 데 그치지만, 반대로 틀리면 사용자가 보려던 결과가 사라진다. 애매한 `EXEC`·`CALL` 을 결과셋 쪽에 두는 이유도 같다.
- 판단은 **배치 안의 모든 문장**을 본다(첫 키워드만 보지 않는다). `UPDATE …; SELECT …` 처럼 섞인 배치도 결과가 남는다. 괄호 안(`WHERE x IN (SELECT …)`)은 독립 문장이 아니므로 세지 않는다.
- 스크립트 도중 연결이 끊기면 **되살려 재시도하지 않는다.** 어디까지 반영됐는지 알 수 없어 중복 적용 위험이 있다. 첫 배치의 조회에서만 한 번 재연결한다(아직 아무것도 반영되지 않았음이 확실한 경우).

sqlx 쪽(PG·MySQL·SQLite)은 `fetch_many` 가 결과셋과 행 수를 한 스트림에 섞어 주므로 이런 판단이 필요 없다.
...
- 키워드 완성은 `@codemirror/lang-sql` 이 방언별로 제공한다. **테이블·컬럼 완성은 직접 만든 소스**(`src/lib/sqlCompletion.ts`)가 맡는다.
  ...
  - **`db.schema.table.column` 4단계를 모두 푼다.** 어느 단계인지 자리로 정하지 않고 **이름이 무엇과 맞는지로** 판단한다 — 사용자가 `dbo.` 로 시작할 수도, DB 부터 적을 수도, 테이블만 적을 수도 있어 위치만으로는 알 수 없다.
  - 그 외(접두어 없음)에는 테이블·DB·스키마 목록과 함께 **지금 FROM 절에 있는 테이블들의 컬럼**을 올린다(SELECT 절에서 컬럼을 쓰려면 이게 있어야 한다). 별칭도 원래 테이블로 풀어 준다.
  - 스냅샷은 **스키마를 지정하지 않고** 받는다. `db.schema.table` 로 쓰려면 그 DB 의 스키마를 다 알고 있어야 하기 때문이다.
  - 스키마 후보에는 `boost` 음수를 준다 — `sel` 처럼 키워드를 치는 중에 이름이 우연히 매칭되어 목록 위를 차지하면 방해가 된다.
...
- 값을 보여 주는 대신 **범위를 좁혀 둔다**: 로그는 메모리에만 있고(최근 300건), 디스크 기록도 외부 전송도 없다(§9). 비밀번호는 애초에 이 경로를 지나지 않는다.
