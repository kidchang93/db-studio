...
- 키워드 완성은 `@codemirror/lang-sql` 이 방언별로 제공한다. **테이블·컬럼 완성은 직접 만든 소스**(`src/lib/sqlCompletion.ts`)가 맡는다.
  - `lang-sql` 의 `sql({schema})` 를 쓰지 않은 이유는 **그 설정이 정적이기 때문**이다. 스냅샷이 도착할 때마다 확장을 새로 만들어야 하고, 그러면 에디터가 통째로 재구성되어 커서·실행취소 이력이 날아가고 입력이 끊긴다. 커스텀 소스는 `ref` 로 최신 스키마를 읽으므로 **확장이 고정**된다.
  - 제안 규칙: `별칭.`/`테이블.` 뒤에는 그 테이블의 컬럼, 그 외에는 테이블 목록 + **지금 FROM 절에 있는 테이블들의 컬럼**(SELECT 절에서 컬럼을 쓰려면 이게 있어야 한다).
  - 참조 테이블은 파서 대신 정규식으로 `FROM`/`JOIN`/`UPDATE`/`INTO` 뒤를 훑는다 — 편집 중인 SQL 은 대개 문법이 깨져 있어 파싱이 실패하고, 그때도 완성은 떠야 한다. 별칭 그룹에는 **negative lookahead 가 필수**다(`FROM a JOIN b` 에서 `JOIN` 을 별칭으로 삼키면 `b` 를 놓친다).
